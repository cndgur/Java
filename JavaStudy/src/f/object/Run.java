...
/*
 	package
 	클래스간의 공간적 충돌이나 접근방법이ㅢ 충돌을 막기위해
...
public class Run {
	 ...
	 * 3) 추출한 것들을 가지고 내가 구현하고자하는 프로그램의 "실질적인 목적"에 맞춰 불필요한 속성, 기능 제거할 것
	 * 4) 최종적으로 추려진 속성들을 어떤 자료형으로, 어떤 변수명으로 사용할 것인지 정의
	 * 
	 * ex) 학생 관리 프로그램 
	 * 1) 학생관련 객체(최경채훈련생, 최용준훈련생, 안재휘훈련생등등...)
	 * 2) 공통적인 속성 및 기능(이름, 나이, 주소, 전화번호, 키, 몸무게, 머리색, 발사이즈,달리기 속도, 먹는다, 잔다, 화장실간다, 공부한다, 휴식한다)
	 ...
	 *	  안재휘라는 훈련생 객체 하나 만들기 -> String name2 = "안재휘"; int age = 19; double height = 193; 
	 * 2. 배열로 프로그래밍 한다면
	 ...
	 * 		  외부로부터의 "데이터 접근을 막고"
	 * 		  대신에 "데이터를 간접적으로 처리(값을 변경하거나,가져오는 것)"할 매소드들을 클래스 내부에 작성하여 관리하는 것
