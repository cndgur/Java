...
public class Book {
	/*
	 	...
	 	default, public
	  	...
	  	default : 같은 패키지 일때만 접근 가능
	  	provate : 오직 해당 클래스에서만 가능
	 */
	...
	//객체를 생성할때 데이터를 초기화 해주는 작업이 필수적으로 필요하기에
	//초기화를 위한 매소드를 만들어준다 -> 생성자라는 규칙이 생김 
//	public void init(String title, String genre, String author, Int maxPage) {
//		this.title = title; //내가가진 (this)에 넘겨받은
//		this.genre = genre;
//		this.author = author;
...
//
	/*
	 생성자
	 - 클래스의 이름과 동일한 메소드로 변화형이 없다
	 - 필드 데이터의 초기화를 위한 특수목저의 메소드이다.
	 - 매개변수가 하나도 없는 생성자를 우리는 기본생성자라고합니다.
	 
	 - 생성자를 개발자가 직접 만들지 않는다면 컴파일러는 기본생성자를 만들어준다.
	 - 개발자가 직접 생성자를 작성하면 기본생성자를 만들어주지 않는다.
	 - 메소드의 매개변수의 객수 또는 타입에 따라서 메소드를 구분하는 메소드 오버로딩은 생성자에도 적용이 된다.
	 */
	public Book() {}	
