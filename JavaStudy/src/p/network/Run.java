...
/*
 	...
 	IP주소 : 네트워크 상에 각 컴퓨터들을 식별해줄 수 있는 번호(실제 주소와 동일한 역할을 한다고 생각하면 된다.)
 	...
 	*클라이언트 : 서버에 요청을 하는 컴퓨터(서비스를 제공받는 고객)
 	...
 	-서버에 요청하기 위해서는 요청하고자하는 서버에 IP주소(또는 도메인), 포트번호를 알아야함
 	...
 	* 현재 구동중인 서버가 있을 때 
 	  ...
 	  *자바만 가지고 서버와 클라이언트의 간단한 통신을 해보자
 	   이 때 데이터를 입출력하고자 한다면 서버와 클라이언트간에 스트림(통로)이 필요함.
 	   
 	   소켓 프로그래밍(TCP / UDP)
 	   - TCP방식 : 데이터의 전송속도가 느려! 데이터가 정확하고 안정적으로 전달이 가능합니다.(신뢰성이 요구되는 프로그램)
 	   - UDP방식 : 데이터의 전송속도가 빠름! 신뢰성이 없는 데이터 전달에 용이(데이터를 빠른속도로 전송하고자 할 때)
 */
...
public class Run {
	public static void main(String[] args) {
		try {
			...
			System.out.println("내 IP 주소명 : " + localhost.getHostAddress()); // 호스트의 IP주소를 반환
			
			System.out.println("=================================");
			
			InetAddress googleHost = InetAddress.getByName("www.google.com");// 도메인명을 통해서 해당 호스트의 정보를 가져올 수 있다.
			
			System.out.println("google 서버명 : " + googleHost.getHostName()); // google 이름을 반환
			System.out.println("google IP 주소 : " + googleHost.getHostAddress()); // google IP주소를 반환
			...
			for(InetAddress n : naverHost) {
				System.out.println("google 서버명 : " + n.getHostName()); // naver 이름을 반환
				System.out.println("google IP 주소 : " + n.getHostAddress()); // naver IP주소를 반환
			}
