...
public class TCPServer {
	...
	public static void main(String[] args) {
		...
		PrintWriter pw = null;
		//1)포트번호 지정(서버 측에서 몇번 포트로 통로를 열것인가?)
		int port = 3000;
		...
		try {
			...
			//3)요청을 기다렸다 요펑이 오면 -> 요청 수락 후 클라이언트와 통신할 수 있느 서버쪽 소켓객체 생성
			...
			//입력용 스트림(클라이언트로부터 전달된 값을 한줄단위로 읽어드리기 위해서 입력용 스트림사용)
		...
		while(true) {	
			...
			pw.print(sendMessage);// 클라이언트에게 출력
