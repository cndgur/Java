...
public class Run {
	/*
	 	...
	 	이 때 반드시 외부배체와 연결되는 통로를 만들어줘야 한다. => 스트림
	 	
	 	* 스트림의 특징
	 	* - 단방향 : 입력이면 입력 / 출력이면 출력만 가능
	 	* 			=> 입력과 출력을 동시에 하고자 한다면 -> 입력스트림, 출력스트림을 따로 만들어 열어줘야한다.
	 	* - 선입선출(FIFO) : 통로가 파이프같은 개념이기때문에 먼저 보낸값이 먼저 나온다.
	 	...
	 	* 스트림의 구분
	 	...
	 	- 보조스트림 : 기반스트림을 보조하는 통로(속도를 빠르게하거나, 유용한 기능을 제공하기위한 보조역할)
